...
Public Class mapMoons
    Inherits SDEFilesBase
    Implements IImporter(Of Moon)
    Implements IDatabaseImporter(Of Moon)

    Public Const BaseFileName As String = "mapMoons"
    ...
    ''' </summary>
    Public Function ImportFile() As Dictionary(Of Long, Moon) Implements IImporter(Of Moon).ImportFile

        FileNameErrorTracker = BaseFileName

        Return ImportJsonlFile(Of moon)(SDEFilePath, Function(c) c._key)
    ...
    Public Sub InsertDatatoDatabase(Records As Dictionary(Of Long, Moon), ByVal Params As ImportParameters) _
        Implements IDatabaseImporter(Of Moon).InsertDatatoDatabase
        FileNameErrorTracker = BaseFileName
        ...
        Dim Table As New List(Of DBTableField) From {
            New DBTableField("moonID", FieldType.int_type, 0, False, True),
            New DBTableField("moonName", FieldType.nvarchar_type, 250, True), ' Set later
        ...
        Table = New List(Of DBTableField) From {
            New DBTableField("moonID", FieldType.int_type, 0, False, False),
        ...
        Call UpdateDB.CreateIndex(TableName & "NPCStationIDs", "IDX_" & TableName & "NPCStationIDs" & "_CID", IndexFields)
        ...
        For Each DataField In Records
            ...
            With DataField.Value
                ...
                DataFields.Add(UpdateDB.BuildDatabaseField("moonID", DataField.Key, FieldType.int_type))
                ...
                If Not IsNothing(.npcStationIDs) Then
                    For Each station In .npcStationIDs
                        DataFields2 = New List(Of DBField) From {
                            UpdateDB.BuildDatabaseField("moonID", DataField.Key, FieldType.int_type),
...
Public Class moon
    ...
    Public Property attributes As MoonAttributes
    Public Property celestialIndex As Integer
    Public Property npcStationIDs As List(Of Long)
    Public Property orbitID As Long
